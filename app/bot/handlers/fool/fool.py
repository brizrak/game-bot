...
from app.bot.handlers.states2 import FoolStates
...
def stich_generic_trump_field_deck_message(given_deck, given_field, trump_type, given_leftover_deck, gid):
    ldeck = 0
    if len(given_leftover_deck) < 3:
    ...
    for i in range(ldeck):
        ...
        pos = (50, 50 + 5 * i)  # Позиция для первого маленького изображения
        large_image.paste(small_image, pos)
        
    pos_player = (0, 0)
    pos_field = (0, 0)
    center = 960 - 50 - (25 * len(given_deck))
    center_field = 960 - 50 - (125 * len(given_field))
    index = 0
    for el in given_deck:
    ...
    for element in given_field:
        ...
        for elel in element:
            ...
            # Определяем позиции, где будем размещать маленькие изображения
            pos_field = (center + (250 * index), 255 + (45 * microindex))  # Позиция для первого маленького изображения
            microindex += 1
            # Накладываем маленькие изображения на большое
    ...
    if not(ignore_deck):
        ...
        for i in range(len(given_deck)):
            card_list+=f"{given_deck[i][1]} {type_to_ru_adj_string(given_deck[i][0])};  "
            if not((i+1)%4) or i == len(given_deck)-1:
                card_list+="\n"
        the_message+=card_list
    return the_message

@router.message(F.text == "/play")
async def play_start_fool(message: Message, state: FSMContext):
...
async def start_game(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    message = stich_generic_trump_field_deck_message(player_deck, field, trump_type, deck, slovar["Айди"])
...
async def ai_turn(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if (len(player_deck) > 0 and len(comp_deck) > 0) or len(deck) or slovar["comp_takes"] or slovar["player_takes"]:
        ...
        while should_get_a_turn > 0:            
            ...
            if not(attacker) and len(player_deck):
                ...
                for i in range(len(comp_deck)):
                    if i >= len(comp_deck):
                        break
                    if (not(len(field)) or target_is_valid_card_to_play(comp_deck[i], field, trump_type, not(attacker))):
        ...
        if len(player_deck):
            ...
            #UserData.balance -= 100
            #await RedisStorage.update_user(UserData.id, UserData)
            await message.answer(f"Игра окончена, вы проиграли!", reply_markup=weirdfinal_kb)
        else:
            ...
            #UserData.balance += 100
            #await RedisStorage.update_user(UserData.id, UserData)
            await message.answer(f"Игра окончена, вы победили!", reply_markup=weirdfinal_kb)
        await state.update_data(slovar)
        await state.set_state(FoolStates.itog)
...
async def play_card(query: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    for i in range(len(player_deck)):
        if not(len(field)) or target_is_valid_card_to_play(player_deck[i], field, trump_type, attacker):
            builder.add(InlineKeyboardButton(text=f"{player_deck[i][1]} {type_to_ru_adj_string(player_deck[i][0])}", callback_data = f"cardnum_{i+1}"))
    builder.adjust(3)
    await query.answer()
